...
TEST(OneHotEncoderTest, Mlir) {
  ...
  auto mlir_script = R"mlir(
    func @main(%path: !tfrt.string, %inputs: !clink.modelinput) -> !clink.modeloutput {
      %model = clink.load.onehotencoder %path
      %outputs = clink.transform %model, %inputs
      tfrt.return %outputs : !clink.modeloutput
    }
  )mlir";
  ...
  inputs.push_back(tfrt::MakeAvailableAsyncValueRef<llvm::ArrayRef<tfrt::AsyncValue *>>(inputs_vec));
  ...
  SparseVector& vector = results[0]->get<llvm::SmallVector<tfrt::AsyncValue *, 1>>()[0]->get<SparseVector>();
  EXPECT_EQ(vector.toString(), "(2, (1), (1.000000)");
}
