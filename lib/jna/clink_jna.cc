...
#ifndef NDEBUG
  static std::unique_ptr<tfrt::HostContext> jna_host_context =
      clink::CreateHostContext("mstd",
                               tfrt::HostAllocatorType::kLeakCheckMalloc);
#else
  static std::unique_ptr<tfrt::HostContext> jna_host_context =
      clink::CreateHostContext("mstd", tfrt::HostAllocatorType::kMalloc);
#endif
  return jna_host_context.get();
}
...
typedef struct SparseVectorJNA {
  SparseVectorJNA(const clink::SparseVector &vector, tfrt::HostContext *host_context);
  ~SparseVectorJNA();
